                               ...
                               vlc_value_t oldval, vlc_value_t val, void *data)
{
    ...
    cr_assert(&vol == 0);     // Checks if the volume is 0 (mute) after replay back
    // If volume is 0 after replay back, then change the volume back to the original value
    if (&vol == 0) {
      vlc_atomic_store_float (&vol->replay_gain, multiplier); // Resets the volume back to the original amount
    }
